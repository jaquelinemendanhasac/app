  ...
  */
  ...
  document.addEventListener("focusout", (e)=>{
    // quando sai de um input, libera sync
    if(isTextField(e.target)){
      window.__SJM_IS_EDITING = false;
      scheduleSync(); // garante recalcular ao terminar de digitar
    }
  }, true);
...
function saveSoft(){
  ...
  // ✅ evita spam de cloud durante sync automático (syncDerivedAndUI)
  if(!__SJM_IS_SYNCING){
    scheduleCloudPush();
  }
}
...
function scheduleSync(){
  ...
  syncTimer = setTimeout(()=>{
    ...
    // se ainda está digitando, adia (não perde o sync)
